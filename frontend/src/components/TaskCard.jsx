...
const TaskCard = ({task, index, handleTaskChanged}) => {
  ...
  const updateTask = async () => {
    try {
      ...
      console.log("Lỗi xảy ra khi cập nhật task: ", error);
  ...
  const toggleTaskCompleteButton = async () => {
    try {
      if(task.status === 'active'){
        ...
        toast.warning(`${task.title} đã được chuyển sang chưa hoàn thành!`)
      }
      handleTaskChanged()
    } catch (error) {
      console.log("Lỗi xảy ra khi cập nhật trạng thái task: ", error);
      toast.error("Lỗi xảy ra khi cập nhật trạng thái task")
    }
